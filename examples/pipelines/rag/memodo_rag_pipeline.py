...
class Retreive:
    def __init__(self, model, host, port, collection_name, openai_api_key):
        printWithEmphasis(f"Using model: {model}")
    ...
    def lineListOutputParser(self, text: str) -> List[str]:
        ...
        printWithEmphasis(f"Multi-query questions: {lines}")
        return lines  
    ...
    def query_db(self, questions):
        ...
        printWithEmphasis(f"Raw Results: {results}")
        printWithEmphasis(f"Result Documents: {results['documents']}")
        return results["documents"]

    def preview_results(self, results):
        printWithEmphasis(f"Preview Results: {results}")
        return results
    ...
    def do_rag(self, user_question):
        ...
        try:
            printWithEmphasis(f"User's question: {user_question}")
            result = chain.invoke({"question": user_question})
            printWithEmphasis(f"Result: {result}")
...
class Pipeline:
    ...
    async def on_startup(self):
        ...
        self.retreive = Retreive(self.valves.MODEL_NAME, self.valves.VECTOR_DB_HOST, self.valves.VECTOR_DB_PORT, self.valves.COLLECTION_NAME, self.valves.OPENAI_API_KEY)
